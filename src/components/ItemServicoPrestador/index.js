...
const ItemServicoPrestador = ( props ) => {
  const token = useSelector(({ authenticate: { token } }) => token);

  const alertServiceApply = () =>{
    Alert.alert(
      "Aplicar candidatura",
      "Deseja realmente realizar esse serviço?",
      [
        {
          text: "Cancelar",
  ...
  const handleApplyService = async () =>{
    const url = 'api/Servico/service-apply';
    ...
    await api
      ...
      .then(async ( { data } ) => {  
        showMessage({
          message: "Sucesso!",
          description: "Candidatura aplicada com sucesso!",
          type: "success",
          duration: 2350,
          icon: "auto"
        });
      ...
      .catch((err) => {
          ...
          showMessage({
            message: "Erro!",
            description: "Não foi possível realizar a candidatura no serviço!",
            type: "danger",
            duration: 2350,
            icon: "auto"
          });
...
export default ItemServicoPrestador;
